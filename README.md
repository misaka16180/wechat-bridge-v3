...
- 不依赖桥接启停的 Agent 主动发送 API，包含幂等 `request_id`、排队位置、阶段进度和日志；图片和文件可先上传二进制，再用一次性 `upload_id` 创建任务。
- 文字、图片、文件和群聊真实 `@` 的可见桌面发送。
- 逐字输入可选长文本自动切换：默认阈值为 40，超过阈值时只把普通文本改为剪贴板粘贴，真实 `@` 保持候选框流程。
- 逐字输入可选随机分组和思考停顿，鼠标使用带轻微扰动的弧线路径；发送前可设置独立的随机检查停顿。
- 内置完整 Agent 接入文档；可显式开启默认关闭的公开页，让 Agent 无需登录读取网页或原始 Markdown。
- 默认通过视觉分组定位进入会话：搜索框限定横向范围，识别“最常使用 / 功能 / 联系人 / 群聊”等标题，按纵坐标选择最上方分组并点击第一项；纯键盘快捷操作仅保留为兼容选项。
- 输入会话名称后默认随机等待 `0.5–0.7` 秒，图像识别耗时计入预算；提前识别成功也会在等待结束后重新截图确认，避免点击仍在加载中的旧结果。
...
下面用一张宫格快速展示最核心的四个页面：运行总览、自动化配置、现场调试和视觉识别。
图中保留了真实的电脑网页端界面；连接设置、运行日志、内置说明、基础设置以及完整操作顺序见
[《界面与操作图集》](docs/界面操作图集.md)。

![电脑端控制台核心界面预览](docs/images/product-overview-grid.png)

从左上到右下依次是：桥接与微信自动化的独立状态、DPI 与窗口运行环境、文字/图片/文件调试及耗时日志、
视觉兼容性检查与候选修复。完整原尺寸截图也已随图集公开，方便进一步查看文字和操作细节。
...
| Agent 主动发送 | 通过 `/api/v1/messages` 异步提交文字、图片或文件；媒体优先上传到 `/api/v1/uploads`；桥接未启动或已停止时仍可排队执行 | 使用独立 Token 和 `request_id` 幂等；必须启动微信自动化，但不套用被动回复的“收到消息后最短间隔”；公开说明页默认关闭 |
| 桌面发送 | 搜索会话、识别最上方搜索分组并点击第一项、定位输入区、输入文字/真实 `@`、粘贴图片或文件、点击发送按钮；逐字输入可按可调阈值把长文本自动切换为粘贴 | 找不到合格分组标题时保存快照并停止，不回退固定坐标或盲按 Enter；真实 `@` 不参与长文本粘贴；只在发送提交前整体重试，提交后即使未看到气泡也不自动重发 |
...
- 不通过文字识别消歧同名会话或同名群成员，默认快捷键方式和鼠标备用方式都会选择微信当前排列的第一项。
...
- “公开 API 文档（无需登录）”默认关闭；只在 Agent 需要直接读取时开启。局域网监听模式下，开启后同网段设备都能阅读静态接口说明，但页面不会包含 Token、任务或日志。
