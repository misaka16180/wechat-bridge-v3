...
- 默认通过纯键盘快捷操作进入会话：输入名称后按一次上方向键，再按 Enter 进入最上方结果；固定区域鼠标点击只作为手动选择的不稳定备用方案。
...
| 桌面发送 | 搜索会话、默认用上方向键＋Enter 进入最上方结果、定位输入区、输入文字/真实 `@`、粘贴图片或文件、点击发送按钮；逐字输入可按可调阈值把长文本自动切换为粘贴 | 固定结果区鼠标点击仅是不稳定备用方案；真实 `@` 不参与长文本粘贴；只在发送提交前整体重试，提交后即使未看到气泡也不自动重发 |
